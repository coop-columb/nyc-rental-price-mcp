...
class Scraper:
    ...
    def parse_listings(self, html_content: str) -> List[Dict[str, Any]]:
        ...
        for element in listing_elements:
            listing = {
                ...
                'price': element.select_one('.price').text.strip() if element.select_one('.price') else '',
                'location': element.select_one('.location').text.strip() if element.select_one('.location') else '',
                'bedrooms': element.select_one('.bedrooms').text.strip() if element.select_one('.bedrooms') else '',
                'bathrooms': element.select_one('.bathrooms').text.strip() if element.select_one('.bathrooms') else '',
                'url': element.select_one('a')['href'] if element.select_one('a') else '',
            }
    ...
    def search(self, query: str, location: Optional[str] = None, 
               min_price: Optional[int] = None, max_price: Optional[int] = None) -> List[Dict[str, Any]]:
        """
